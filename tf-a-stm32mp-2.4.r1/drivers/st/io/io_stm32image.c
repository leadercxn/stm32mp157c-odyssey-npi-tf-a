...
static int stm32image_partition_size(io_entity_t *entity, size_t *length)
{
	...
	{
		ERROR("%s: partition %s not found at %x\n",
			__func__, current_part->name, *stm32_img);
		result = -ENOMEM;

		if(header->magic != BOOT_API_IMAGE_HEADER_MAGIC_NB)
		{
			ERROR("header->magic != BOOT_API_IMAGE_HEADER_MAGIC_NB\n");
		}

		if(header->binary_type != current_part->binary_type)
		{
			ERROR("header->binary_type(%u) != current_part->binary_type(%u)\n",header->binary_type,current_part->binary_type);
		}

		if(header->image_length >= stm32image_dev.device_size)
		{
			ERROR("header->image_length >= stm32image_dev.device_size\n");
		}
